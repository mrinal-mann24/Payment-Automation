...
describe("parseEmailList", () => {
  it("splits on commas, semicolons and whitespace, keeps only real addresses, drops duplicates", () => {
    expect(parseEmailList("accounts@acme.example, cfo@acme.example;  NA  ceo@acme.example\naccounts@acme.example")).toEqual([
      "accounts@acme.example",
      "cfo@acme.example",
      "ceo@acme.example",
    ]);
    expect(parseEmailList(",sencer@example.com")).toEqual(["sencer@example.com"]);
    for (const junk of [null, undefined, "", "NA", "--", "98911 46116", " , ; "]) {
      expect(parseEmailList(junk)).toEqual([]);
    }
  });
});

describe("fetchDealWithLineItemsAndContact — billing emails", () => {
  it("collects every address in the Accountant Email list; the contact stays the Zoho identity", async () => {
    mockDealFetch({
      accountantEmail: "accounts@acme.example ,cfo@acme.example; NA",
  ...
  it("has no billing email at all when the field is blank or holds no address — never the contact's, never the POC's", async () => {
    for (const value of [null, "", "NA", "--", "98911 46116"]) {
      mockDealFetch({ accountantEmail: value, pocEmail: "poc@acme.example", contact: { email: "owner@acme.example" } });

      const deal = await fetchDealWithLineItemsAndContact("deal-1");

      expect(deal.billingEmails).toEqual([]);
      expect(deal.contactEmail).toBe("owner@acme.example");
    }
  });

  it("uses the first listed address as the Zoho identity when the deal has no associated contact", async () => {
    mockDealFetch({ accountantEmail: "cfo@acme.example, ceo@acme.example", pocName: "Ayurpet", contact: null });

    const deal = await fetchDealWithLineItemsAndContact("deal-1");

    expect(deal.contactEmail).toBe("cfo@acme.example");
    expect(deal.billingEmails).toEqual(["cfo@acme.example", "ceo@acme.example"]);
...
describe("updateDealAccountantEmails", () => {
  it("writes the addresses as one comma-separated value, and clears the field with an empty string", async () => {
    const calls = mockDealFetch({ contact: null });

    await updateDealAccountantEmails("deal-1", ["accounts@acme.example", "cfo@acme.example"]);
    await updateDealAccountantEmails("deal-1", []);

    const patches = calls.filter((c) => c.method === "PATCH");
    expect(patches.map((c) => c.body)).toEqual([
      { properties: { accountant_email: "accounts@acme.example, cfo@acme.example" } },
...
describe("fetchVaDealEmails", () => {
  it("reads every deal's Accountant Email in one batch call; a deal missing from the result is left blank", async () => {
      ...
      if (path.endsWith("/crm/v3/objects/deals/batch/read")) {
        ...
        return jsonResponse({
          results: body.inputs.filter(({ id }) => id !== "d3").map(({ id }) => ({
            id,
            properties: { accountant_email: id === "d1" ? "accounts@one.example, cfo@one.example" : "NA" },
          })),
        });
      }
      throw new Error(`Unexpected fetch: ${path}`);
    ...
    expect(emails.get("d1")).toEqual({ accountantEmail: "accounts@one.example, cfo@one.example" });
